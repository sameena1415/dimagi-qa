...
class ExportDataPage(BasePage):
    ...
    def __init__(self, driver):
        ...
        self.update_data_conf =  "(//*[contains(@data-bind,'hasEmailedExport')][.//span[.='{}']]/following-sibling::div//button[contains(@data-bind,'visible: emailedExport.canUpdateData')])[1]"
    ...
    def excel_dashboard_integration_form(self):
        ...
        self.wait_for_element((By.XPATH, self.update_data_conf.format(UserData.dashboard_feed_form)), 30)
        self.js_click((By.XPATH, self.update_data_conf.format(UserData.dashboard_feed_form)))
    ...
    def excel_dashboard_integration_case(self):
        ...
        # self.wait_till_progress_completes("integration")
        self.wait_for_element((By.XPATH, self.update_data_conf.format(UserData.dashboard_feed_case)), 30)
        self.js_click((By.XPATH, self.update_data_conf.format(UserData.dashboard_feed_case)))
        assert self.is_visible_and_displayed(self.data_upload_msg), "Export not completed!"
        time.sleep(2)
        self.driver.refresh()
        time.sleep(2)
