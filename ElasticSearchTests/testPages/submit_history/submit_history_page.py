...
class SubmitHistoryPage(BasePage):
    ...
    def verify_multiple_users(self):
        ...
        assert self.is_visible_and_displayed(self.report_content_id, 120), "Report not loaded"
        print("Report loaded successfully!")
        self.verify_users_used_in_the_group([UserData.app_login, UserData.web_user])
        time.sleep(10)
        self.scroll_to_element((By.XPATH, self.custome_remove_button.format(UserData.web_user)))
        self.click((By.XPATH, self.custome_remove_button.format(UserData.web_user)))
        time.sleep(2)
        ActionChains(self.driver).send_keys(Keys.TAB).perform()
        time.sleep(2)
        self.wait_to_click(self.apply_id)
        ...
        self.verify_users_used_not_in_the_group([UserData.web_user])
